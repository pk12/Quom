...
public class TimeAttack extends Mode {
    ...
    public TimeAttack(String category, Context context,TextView time) {
        super();
        ...
        this.time = time;
        timer = (Timer) new Timer(121000,1000,time).start();
    }
...
//            multip = 1;
    ...
    public void timerReset(){
        ...
        timer = new Timer(seconds * 1000,1000,time); //if i use the reset function the old one does not stop for some reason if it is cancelled in the class
        timer.start();
    }
}
